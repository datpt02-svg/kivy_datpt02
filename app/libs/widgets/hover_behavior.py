        ...
        if self.allow_hover:
            ...
            while True:
                ...
                try:
                    ...
                    # If parent has no opacity or is a BoxLayout, treat it as not covering.
                    if parent.opacity == 0 or isinstance(parent, BoxLayout):
                        widget = parent
                        continue
            ...
            if parent:
                for child in parent.children:
                    ...
                    if isinstance(child, Widget):
                        ...
                        # Skip TouchBlocker if it explicitly allows this widget
                        if child.__class__.__name__ == "TouchBlocker":
                            if any(
                                w.collide_point(*w.to_widget(*pos))
                                for w in getattr(child, "allow_widget", [])
                            ):
                                continue   # skip blocking, allow hover
